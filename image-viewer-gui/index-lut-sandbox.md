...
인덱스/LUT 샌드박스는 Chloros 이미지 뷰어 내의 대화형 작업 공간으로, 다중 스펙트럼 인덱스 계산과 컬러 시각화를 실시간으로 실험할 수 있게 해줍니다. 이 강력한 도구를 사용하면 전체 데이터셋을 재처리하지 않고도 다양한 인덱스를 테스트하고, 값 범위를 조정하며, 출판 가능한 시각화를 생성할 수 있습니다.
...
1. 사이드바 상단의 인덱스(Index) 상자를 선택하세요
2. 왼쪽 드롭다운에서 카메라의 필터를 선택하세요
...
전체 인덱스 공식 문서는 [다중 스펙트럼 인덱스 공식](../project-settings/multispectral-index-formulas.md)을 참조하십시오.
...
1. <img src="../.gitbook/assets/image.png" alt="" data-size="line"> &quot;+LUT 추가&quot; 버튼을 클릭합니다
2. 색상 그라데이션을 선택합니다
3. 클리핑 최소/최대 종점을 조정합니다
4. 클리핑 모드를 조정합니다
5. **이미지 뷰어** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> 탭 사이드바의 **색상 지수** 상자를 선택하여 LUT 적용

### 색상 그라데이션 선택

**그라데이션 선택 방법:**

1. LUT 패널에서 **색상 그라데이션 바**를 찾습니다
2. 마우스를 올려놓으면 사용 가능한 그라데이션 프리셋을 볼 수 있습니다
3. 원하는 그라데이션 선택
4. 인덱스 상자 체크 시 이미지가 **즉시 업데이트**되어 새 색상 적용

{% hint style=&quot;success&quot; %}
**권장 사항**: NDVI와 같은 식생 지수의 경우, Red-Yellow-Green 그라데이션이 자연스러운 색상 연상(녹색=건강, 노란색=보통, 빨간색=스트레스)과 일치하므로 가장 직관적입니다.
{% endhint %}

### 색상 클래스 조정

**클래스 제어**는 그라데이션에 나타나는 개별 색상 단계의 수를 결정합니다:
...
* **적은 클래스** (3-5): 뚜렷한 구역 생성, 단순화된 분류, 범주 구분 용이
* **중간 클래스** (6-10): 균형 잡힌 접근, 대부분의 용도에 적합
* **많은 클래스** (15-20): 부드러운 전환, 세밀한 변형, 사진 같은 표현

**사용 시점:**

* **적은 클래스 (3-5)**: 프레젠테이션 슬라이드, 분류 지도, 간단한 보고서
* **중간 클래스 (6-10)**: 일반 분석, 균형 잡힌 세부 사항, 표준 보고서
* **다수 클래스 (15-20)**: 과학적 분석, 상세 검사, 출판물 수준의 결과물

### 값 범위 미세 조정

**값 범위 제어**는 그라데이션에서 색상 지수 값이 어떤 색상에 매핑되는지 결정합니다:
...
* 관심 영역의 **대비 강화**
* **관련 없는 값 제외** (예: 수역, 노출된 토양)
* 여러 이미지 또는 날짜 간 **시각화 표준화**
* 좁은 값 범위 내 **미세한 차이 강조**
...
* **사용 사례**: 미묘한 강조 효과, 관심 영역을 부각시키면서 맥락 유지
* **예시**: 스트레스 받은 식생(NDVI 0.3-0.5)을 색상으로 강조하고 건강한 영역은 회색으로 표시

#### 4. 원본 배경

* **범위 외** 픽셀은 **원본 다중 스펙트럼 이미지** 표시
* **범위 내** 픽셀은 **색상 그라데이션** 표시
* **사용 사례**: 가장 직관적 - 자연스러운 이미지 컨텍스트와 분석적 색상 오버레이 결합
* **예시**: 실제 필드/작물 외관을 확인하면서 색상 코딩된 스트레스 영역 오버레이 적용
...
| **색상 지수 배경**       | 미묘한 강조, 데이터 컨텍스트 유지  | 범위 내 색상, 범위 외 회색  |
| **원본 배경**    | 보고서, 프레젠테이션, 직관적 분석 | 범위 내 색상, 범위 외 사진 |
...
2. 그라데이션 아래의 **색상 견본 사각형**을 찾습니다
3. **색상 정지점을 클릭**하여 선택합니다
4. **색상 선택기**가 열립니다
5. 다음 방법으로 새 색상을 선택하세요:
   * **컬러 휠**: 시각적 색상 선택
   * **HSV 슬라이더**: 정밀한 색상 제어
   * **16진수 코드 입력**: 정확한 색상 지정 (예: 빨간색 `#FF0000`)
6. 색상 선택기 외부를 클릭하여 **새 색상을 적용**합니다
...
* **브랜드 색상**: 보고서용 조직의 색상 팔레트에 맞추기
* **색맹 친화적**: 주황-파랑 또는 보라-노랑 조합 사용
* **인쇄 최적화**: 컬러 및 그레이스케일 인쇄 모두에 적합한 색상 선택
* **다중 임계값**: 특정 값 임계값에서 분류를 위해 서로 다른 색상 사용
...
* **레이어 전환** → 이미지 즉시 변경
* **그라디언트 선택** → 색상이 즉시 업데이트됩니다
* **값 범위 조정** → 대비가 실시간으로 변경됩니다
* **클래스 변경** → 그라디언트 부드러움이 즉시 업데이트됩니다
* **클리핑 수정** → 배경 표시가 즉시 변경됩니다
* **색상 편집** → 사용자 정의 그라디언트가 즉시 적용됩니다

**&quot;적용&quot; 버튼이 필요 없음** - 모든 변경 사항은 실시간으로 상호작용됩니다!
...
2. **지수 적용** - 카메라 필터와 지수 공식을 선택하고, 색상 원을 지수 공식 내 적절한 위치로 드래그
3. **LUT 그라데이션 적용** - Red-Yellow-Green 프리셋으로 시작
4. **픽셀 값 확인** - 커서를 이동하며 값 범위를 기록
5. **최소/최대값 조정** - 식생에 집중하도록 범위를 좁힘 (예: 0.2~0.9)
6. **클리핑 선택** - 맥락 파악을 위해 &quot;Original Background&quot; 시도
7. **색상 세밀 조정** - 특정 강조를 위해 필요 시 그라데이션 맞춤 설정
8. **설정 확정** - 설정 기록 및 내보내기 처리용 프로젝트 설정으로 복사

### 픽셀 값 확인

효과적인 LUT 범위 설정을 위해 실제 픽셀 값 이해가 중요합니다:

**값 확인 방법:**

1. 이미지의 인덱스 상자 또는 인덱스와 LUT 상자 모두 **선택** 시 픽셀 값이 표시됩니다.
2. 커서를 이미지의 다양한 영역 위로 **이동**하세요
...
2. **인덱스 공식 드롭다운**으로 이동
3. **&quot;사용자 정의&quot;** 옵션 찾기(Chloros+ 라이선스로 로그인 필수)
...
{% hint style=&quot;warning&quot; %}
**공식 유효성 검사**: 공식이 카메라에서 사용 가능한 밴드를 사용하는지 확인하십시오. 예를 들어, RedEdge는 RedEdge 필터가 장착된 카메라에서만 사용 가능합니다.
{% endhint %}
...
* **추가 정보**: 다음 문서 참조 [다중 스펙트럼 인덱스 공식](../project-settings/multispectral-index-formulas.md)
