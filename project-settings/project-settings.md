# 프로젝트 설정

프로젝트 설정 <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> 사이드바를 통해 이미지 처리, 교정 대상 감지, 다중 스펙트럼 지수 계산 및 프로젝트 내보내기 옵션 등 모든 측면을 구성할 수 있습니다. 이러한 설정은 프로젝트와 함께 저장되며 여러 프로젝트에서 재사용할 수 있도록 템플릿으로 저장할 수 있습니다.

## 프로젝트 설정 접근 방법

프로젝트 설정에 접근하려면:

1. Chloros에서 프로젝트를 엽니다
2. 왼쪽 사이드바의 **프로젝트 설정**  <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> 탭을 클릭합니다
3. 설정 패널에 카테고리별로 분류된 모든 구성 옵션이 표시됩니다

***

## 타겟 감지

이 설정은 Chloros가 이미지 내 교정 타겟을 감지하고 처리하는 방식을 제어합니다.

### 최소 교정 샘플 영역 (px)

* **유형**: 숫자
* **범위**: 0 ~ 10,000 픽셀
* **기본값**: 25 픽셀
* **설명**: 검출된 영역이 유효한 보정 대상 샘플로 간주되기 위해 필요한 최소 영역(픽셀 단위)을 설정합니다. 값이 작을수록 더 작은 대상을 검출하지만 오탐이 증가할 수 있습니다. 값이 클수록 더 크고 선명한 대상 영역이 검출에 필요합니다.
* **조정 시기**:
  * 작은 이미지 아티팩트에 대한 오탐이 발생할 경우 증가
  * 보정 대상이 이미지에 작게 나타나고 탐지되지 않을 경우 감소

### 최소 대상 클러스터링 (0-100)

* **유형**: 숫자
* **범위**: 0 ~ 100
* **기본값**: 60
* **설명**: 보정 대상 감지 시 유사한 색상 영역을 그룹화하는 클러스터링 임계값을 제어합니다. 값이 높을수록 더 유사한 색상이 함께 그룹화되어 대상 감지가 보수적으로 이루어집니다. 값이 낮을수록 대상 그룹 내 색상 변이가 더 허용됩니다.
* **조정 시점**:
  * 보정 대상이 여러 감지로 분할될 경우 증가
  * 색상 변이가 있는 보정 대상이 완전히 감지되지 않을 경우 감소

***

## 처리

이 설정은 Chloros가 이미지를 처리하고 보정하는 방식을 제어합니다.

### 비네팅 보정

* **유형**: 체크박스
* **기본값**: 활성화됨 (체크됨)
* **설명**: 이미지 가장자리의 렌즈 어두워짐을 보정하기 위해 비네팅 보정을 적용합니다. 비네팅은 렌즈 특성으로 인해 이미지의 모서리와 가장자리가 중앙보다 어둡게 보이는 일반적인 광학 현상입니다.
* **비활성화 시점**: 카메라/렌즈 조합이 이미 비네팅 보정을 적용한 경우, 또는 후처리에서 수동으로 비네팅을 보정하려는 경우에만 비활성화하십시오.

### 반사율 보정 / 화이트 밸런스

* **유형**: 체크박스
* **기본값**: 활성화됨 (선택됨)
* **설명**: 이미지에서 감지된 보정 타깃을 사용하여 자동 반사율 보정을 활성화합니다. 이는 데이터셋 전반에 걸쳐 반사율 값을 표준화하고 조명 조건에 관계없이 일관된 측정을 보장합니다.
* **비활성화 시점**: 보정되지 않은 원본 이미지를 처리하려는 경우 또는 다른 보정 워크플로를 사용하는 경우에만 비활성화하십시오.

### 디베이어 방법

* **유형**: 드롭다운 선택
* **옵션**:
  * 고품질(빠름) - 현재 유일하게 사용 가능한 옵션
* **기본값**: 고품질(빠름)
* **설명**: 원시 베이어 패턴 센서 데이터를 풀컬러 이미지로 변환하는 데모사이징 알고리즘을 선택합니다. &quot;고품질(빠름)&quot; 방법은 처리 속도와 이미지 품질 간의 최적 균형을 제공합니다.
* **참고**: 향후 Chloros 버전에서 추가 디베이어 방법이 제공될 수 있습니다.

### 최소 재보정 간격

* **유형**: 숫자
* **범위**: 0~3,600초
* **기본값**: 0초
* **설명**: 보정 대상 사용 간 최소 시간 간격(초)을 설정합니다. 0으로 설정 시 Chloros는 감지된 모든 보정 대상을 사용합니다. 더 높은 값으로 설정 시 Chloros는 최소 이 시간 간격 이상으로 분리된 보정 대상만 사용하므로, 보정 대상 캡처가 빈번한 데이터 세트의 처리 시간을 단축합니다.
* **조정 시기**:
  * 조명 조건이 변동할 때 최대 교정 정확도를 위해 0으로 설정
  * 조명이 일관되고 교정 대상 이미지 획득 빈도가 높을 때 처리 속도 향상을 위해 증가(예: 60-300초)

### 광센서 시간대 오프셋

* **유형**: 숫자
* **범위**: -12 ~ +12시간
* **기본값**: 0시간
* **설명**: 광 센서 데이터 타임스탬프의 시간대 오프셋(UTC 기준 시간 단위)을 지정합니다. PPK(Post-Processed Kinematic) 데이터 파일 처리 시 이미지 캡처와 GPS 데이터 간의 정확한 시간 동기화를 보장하기 위해 사용됩니다.
* **조정 시점**: PPK 데이터가 UTC 대신 현지 시간을 사용하는 경우 현지 시간대 오프셋으로 설정하십시오. 예시:
  * 태평양 표준시: -8 또는 -7 (일광 절약 시간제 적용 여부에 따라)
  * 동부 표준시: -5 또는 -4 (일광 절약 시간제 적용 여부에 따라)
  * 중부 유럽 시간: +1 또는 +2 (일광 절약 시간제 적용 여부에 따라)

### PPK 보정 적용

* **유형**: 체크박스
* **기본값**: 비활성화 (선택 해제)
* **설명**: GPS(GNSS)를 포함하는 MAPIR DAQ 레코더의 후처리 동적(PPK) 보정 사용을 활성화합니다. 활성화 시 Chloros는 프로젝트 디렉터리에 노출 핀 데이터가 포함된 모든 .daq 로그 파일을 사용하여 이미지에 정밀한 지리 위치 보정을 적용합니다.
* **필수 조건**: 프로젝트 디렉터리에 노출 핀 항목이 포함된 .daq 로그 파일이 존재해야 함
* **활성화 시기**: .daq 로그 파일에 노출 피드백 항목이 있는 경우 항상 PPK 보정을 활성화하는 것이 권장됩니다.

### 노출 핀 1

* **유형**: 드롭다운 선택
* **표시 여부**: &quot;PPK 보정 적용&quot;이 활성화되고 핀 1에 노출 데이터가 있을 때만 표시됨
* **옵션**:
  * 프로젝트에서 감지된 카메라 모델명
  * &quot;사용 안 함&quot; - 이 노출 핀 무시
* **기본값**: 프로젝트 구성에 따라 자동 선택
* **설명**: PPK 시간 동기화를 위해 노출 핀 1에 특정 카메라를 할당합니다. 노출 핀은 카메라 셔터가 작동된 정확한 시점을 기록하며, 이는 정확한 PPK 위치 측정에 필수적입니다.
* **자동 선택 동작**:
  * 단일 카메라 + 단일 핀: 카메라를 자동 선택
  * 단일 카메라 + 두 핀: 핀 1이 카메라에 자동 할당
  * 다중 카메라: 수동 선택 필요

### 노출 핀 2

* **유형**: 드롭다운 선택
* **표시 여부**: &quot;PPK 보정 적용&quot; 활성화 시 AND 핀 2에 노출 데이터가 존재할 때만 표시
* **옵션**:
  * 프로젝트에서 감지된 카메라 모델명
  * &quot;사용 안 함&quot; - 이 노출 핀 무시
* **기본값**: 프로젝트 구성에 따라 자동 선택
* **설명**: 듀얼 카메라 설정 시 PPK 시간 동기화를 위해 노출 핀 2에 특정 카메라를 할당합니다.
* **자동 선택 동작**:
  * 단일 카메라 + 단일 핀: 핀 2가 자동으로 &quot;사용하지 않음&quot;으로 설정
  * 단일 카메라 + 두 개의 핀: 핀 2가 자동으로 &quot;사용하지 않음&quot;으로 설정됨
  * 다중 카메라: 수동 선택 필요
* **참고**: 동일한 카메라를 핀 1과 핀 2에 동시에 할당할 수 없습니다.

***

## 인덱스

이 설정은 분석 및 시각화를 위한 다중 스펙트럼 지수를 구성할 수 있게 합니다.

### 지수 추가

* **유형**: 특수 지수 구성 패널
* **설명**: 이미지 처리 중 계산할 다중 스펙트럼 식생 지수(NDVI, NDRE, EVI 등)를 선택 및 구성할 수 있는 대화형 패널을 엽니다. 각각 별도의 시각화 설정을 가진 여러 지수를 추가할 수 있습니다.
* **사용 가능한 지수**: 시스템에는 다음을 포함한 30개 이상의 사전 정의된 다중 스펙트럼 지수가 포함됩니다:
  * NDVI (정규화 차분 식생 지수)
  * NDRE (정규화 차분 RedEdge)
  * EVI (강화 식생 지수)
  * GNDVI, SAVI, OSAVI, MSAVI2
  * 기타 다수 (전체 목록은 [다중 스펙트럼 지수 공식](multispectral-index-formulas.md) 참조)
* **주요 기능**:
  * 사전 정의된 지수 공식 선택
  * 시각화 색상 그라데이션 구성 (LUT - 룩업 테이블)
  * 분석용 임계값 설정
  * 사용자 정의 지수 공식 생성

### 사용자 정의 공식 (Chloros+ 기능)

* **유형**: 사용자 정의 공식 정의 배열
* **설명**: 밴드 연산을 사용하여 사용자 정의 다중 스펙트럼 지수 공식을 생성하고 저장할 수 있습니다. 사용자 정의 공식은 프로젝트 설정과 함께 저장되며 내장 지수와 동일하게 사용할 수 있습니다.
* **생성 방법**:
  1. 지수 구성 패널에서 사용자 정의 공식 옵션을 찾습니다.
  2. 밴드 식별자(예: NIR, Red, Green, Blue)를 사용하여 공식을 정의합니다.
  3. 설명적인 이름으로 공식을 저장하세요
* **공식 구문**: 표준 수학 연산이 지원됩니다. 포함 항목:
  * 산술 연산: `+`, `-`, `*`, `/`
  * 연산 순서를 위한 괄호
  * 밴드 참조: NIR, Red, Green, Blue, RedEdge, Cyan, Orange, NIR1, NIR2

***

## 내보내기

이 설정은 처리된 이미지의 내보내기 형식과 품질을 제어합니다.

### 보정된 이미지 형식

* **유형**: 드롭다운 선택
* **옵션**:
  * **TIFF (16비트)** - 압축되지 않은 16비트 TIFF 형식
  * **TIFF (32비트, 백분율)** - 반사율 값을 백분율로 표시하는 32비트 부동 소수점 TIFF
  * **PNG (8비트)** - 압축된 8비트 PNG 형식
  * **JPG (8비트)** - 압축된 8비트 JPEG 형식
* **기본값**: TIFF (16비트)
* **설명**: 처리 및 보정된 이미지를 저장할 파일 형식을 선택합니다.
* **권장 형식**:
  * **TIFF (16비트)**: 과학적 분석 및 전문 워크플로우에 권장됩니다. 압축 아티팩트 없이 최대 데이터 품질을 유지합니다. 다중 스펙트럼 분석 및 GIS 소프트웨어에서의 추가 처리에 최적입니다.
  * **TIFF (32비트, 백분율)**: 반사율 값을 백분율(0-100%)로 요구하는 워크플로우에 최적. 방사계 측정을 위한 최대 정밀도 제공.
  * **PNG (8비트)**: 웹 뷰잉 및 일반 시각화에 적합. 무손실 압축으로 파일 크기는 작지만, 동적 범위가 감소합니다.
  * **JPG (8비트)**: 가장 작은 파일 크기, 미리보기 및 웹 표시 전용으로 최적. 과학적 분석에 적합하지 않은 손실 압축을 사용합니다.

***

## 프로젝트 템플릿 저장

이 기능은 현재 프로젝트 설정을 재사용 가능한 템플릿으로 저장할 수 있게 합니다.

* **입력 방식**: 텍스트 입력 + 저장 버튼
* **설명**: 설정 템플릿에 대한 설명적인 이름을 입력하고 저장 아이콘을 클릭하세요. 템플릿은 현재 프로젝트의 모든 설정(대상 감지, 처리 옵션, 지수, 내보내기 형식)을 저장하여 향후 프로젝트에서 쉽게 재사용할 수 있습니다.
* **사용 사례**:
  * 다양한 카메라 시스템용 템플릿 생성 (RGB, 다중 스펙트럼, NIR)
  * 특정 작물 유형 또는 분석 워크플로우용 표준 구성 저장
  * 팀 전체에 일관된 설정 공유
* **사용 방법**:
  1. 원하는 모든 프로젝트 설정 구성
  2. 템플릿 이름 입력 (예: &quot;RedEdge Survey3 NDVI 표준&quot;)
  3. 저장 아이콘 클릭
  4. 새 프로젝트 생성 시 템플릿을 불러올 수 있음

***

## 프로젝트 폴더 저장

이 설정은 새 프로젝트가 기본적으로 저장되는 위치를 지정합니다.

* **유형**: 디렉터리 경로 표시 + 편집 버튼
* **기본값**: `C:\Users\[Username]\Chloros Projects`
* **설명**: 새 Chloros 프로젝트가 생성되는 현재 기본 디렉터리를 표시합니다. 다른 디렉터리를 선택하려면 편집 아이콘을 클릭하세요.
* **변경 시점**:
  * 팀 협업을 위해 네트워크 드라이브로 설정
  * 대용량 데이터셋을 위해 저장 공간이 더 많은 드라이브로 변경
  * 연도, 고객, 프로젝트 유형별로 다른 폴더에 프로젝트 정리
* **참고**: 이 설정 변경은 새 프로젝트에만 적용됩니다. 기존 프로젝트는 원래 위치에 유지됩니다.

***

## 설정 지속성

모든 프로젝트 설정은 프로젝트 파일(`.mapir` 프로젝트 형식)과 함께 자동 저장됩니다. 프로젝트를 다시 열면 모든 설정이 종료 시점 그대로 복원됩니다.

### 설정 적용 우선순위

설정은 다음 순서로 적용됩니다:

1. **시스템 기본값** - Chloros에서 정의한 내장 기본값
2. **템플릿 설정** - 프로젝트 생성 시 템플릿을 불러온 경우
3. **저장된 프로젝트 설정** - 프로젝트 파일과 함께 저장된 설정
4. **수동 조정** - 현재 세션 중 변경한 모든 사항

### 설정과 이미지 처리

대부분의 설정 변경(특히 &#x27;처리&#x27; 및 &#x27;내보내기&#x27; 범주)은 새 설정을 반영하기 위해 이미지 재처리를 유발합니다. 그러나 일부 설정은 &#x27;내보내기 전용&#x27;으로 즉시 재처리가 필요하지 않습니다:

* 프로젝트 템플릿 저장
* 작업 디렉터리
* 보정된 이미지 형식 (내보내기 시 적용)

***

## 모범 사례

1. **기본값으로 시작**: 대부분의 MAPIR 카메라 시스템과 일반적인 워크플로우에는 기본 설정이 효과적입니다.
2. **템플릿 생성**: 특정 워크플로우나 카메라에 최적화된 설정을 저장하여 프로젝트 간 일관성을 유지하세요.
3. **전체 처리 전 테스트**: 새로운 설정을 실험할 때는 전체 데이터셋을 처리하기 전에 소량의 이미지 샘플로 테스트하십시오.
4. **설정 문서화**: 카메라 시스템, 처리 유형, 사용 목적을 나타내는 설명적인 템플릿 이름을 사용하십시오(예: &quot;Survey3\_RGB\_NDVI\_Agriculture&quot;).
5. **출력 형식 선택**: 최종 용도에 따라 출력 형식을 선택하세요:
   * 과학적 분석 → TIFF (16비트 또는 32비트)
   * GIS 처리 → TIFF (16비트)
   * 빠른 시각화 → PNG (8비트)
   * 웹 공유 → JPG (8비트)

***

Chloros의 다중 스펙트럼 지수에 대한 자세한 내용은 [다중 스펙트럼 지수 공식](multispectral-index-formulas.md) 페이지를 참조하십시오.
