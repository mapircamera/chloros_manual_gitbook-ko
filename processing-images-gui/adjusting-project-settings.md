...
이미지 처리 전에 워크플로 요구 사항에 맞게 프로젝트 설정을 구성하는 것이 중요합니다. 프로젝트 설정 <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> 패널은 보정, 처리 옵션, 다중 스펙트럼 지수 및 내보내기 형식에 대한 포괄적인 제어를 제공합니다.
...
* ✅ **비네트 보정**: 활성화
* ✅ **반사율 보정**: 활성화 (MAPIR 타겟 이미지 필요)
* ✅ **디베이어 방법**: 표준 (빠름, 중간 품질)
* ✅ **내보내기 형식**: TIFF (16비트)

이미지를 가져온 후 이 기본값으로 바로 처리 시작하세요.

***

## 프로젝트 설정 개요

프로젝트 설정 패널은 여러 범주로 구성됩니다. 아래는 각 섹션의 요약입니다. 전체 문서는 [프로젝트 설정](../project-settings/project-settings.md)을 참조하세요.

### 타겟 감지

이미지에서 보정 타겟을 식별하는 방식을 제어합니다.

**주요 설정:*** **최소 교정 샘플 영역**: 대상 감지 크기 임계값 (기본값: 25 픽셀)
* **최소 대상 클러스터링**: 대상 영역 그룹화 유사도 임계값 (기본값: 60)**조정 시점:**

* 오탐이 발생할 경우 샘플 영역 증가
* 대상이 전혀 감지되지 않을 경우 감소
...
**주요 설정:*** **비네트 보정**: 렌즈 가장자리 어두움 보정 ✅ 권장
* **반사율 보정**: 보정 대상 사용 시 값 정규화 ✅ 권장
* **디베이어 방법**: RAW를 3채널 다중 스펙트럼으로 변환하는 알고리즘
* **최소 재보정 간격**: 보정 타겟 사용 간격 (0 = 모두 사용)**고급 설정:*** **광센서 시간대 오프셋**: PPK 시간 동기화용 (기본값: 0)
* **PPK 보정 적용**: .daq 파일의 GPS/노출 핀 데이터 사용
* **노출 핀 1/2**: 듀얼 카메라 설정 시 카메라를 노출 핀에 할당

### 디베이어링 방법

현재 Chloros에서 제공하는 2가지 디베이어링 방법:

#### 표준 (빠름, 중간 품질)

표준 디베이링은 빠르게 처리되지만 디베이링 색상 노이즈가 나타나 정확도가 낮고 노이즈가 많은 이미지를 생성합니다.

#### 텍스처 인식 (느림, 최고 품질) \[Chloros+ 전용]

텍스처 인식은 고품질의 에지 인식 디베이링과 AI/ML 기반 노이즈 제거 모델을 결합하여 디베이링 노이즈를 거의 완전히 제거합니다. 텍스처 인식 모델 실행에는 GPU 메모리(VRAM)가 필요합니다. 더 빠른 처리를 위해 사용 가능한 VRAM이 4GB 이상일 때 사용을 권장합니다.
...
**주요 지수:*** **NDVI**: 일반 식생 건강 상태 (가장 흔함)
* **NDRE**: RedEdge와 함께 초기 스트레스 감지
* **GNDVI**: 엽록소 농도에 민감
* **OSAVI**: 가시광선 토양과 호환성 우수
* **EVI**: 높은 잎면적지수(LAI) 지역**사용자 정의 공식 (Chloros+ 전용):**
...
* **TIFF (32비트, 백분율)**: 부동소수점 반사율 값 (0.0-1.0 범위)
...
1. 프로젝트 설정 패널에서 원하는 모든 설정 구성
2. 하단의 **&quot;프로젝트 템플릿 저장&quot;** 섹션으로 스크롤
3. 설명적인 템플릿 이름 입력 (예: &quot;Survey3N\_RGN\_Agriculture&quot;)
4. 저장 아이콘 클릭
...
정밀 지리 위치 측정을 위해 GPS가 장착된 MAPIR DAQ 레코더를 사용하는 경우:
...
3. 필요한 경우 **&quot;광센서 시간대 오프셋&quot;** 설정 (기본값: UTC 기준 0)
4. 카메라를 노출 핀에 할당:
   * **단일 카메라**: 자동으로 핀 1에 할당
   * **듀얼 카메라**: 각 카메라를 올바른 핀에 수동 할당**노출 핀 할당:*** **노출 핀 1**: 드롭다운에서 카메라 모델 선택
* **노출 핀 2**: 두 번째 카메라 선택 또는 &quot;사용 안 함&quot; 선택
...
이미지 수가 많은 프로젝트(500장 이상)의 경우:

* 날짜별 또는 지역별로 소규모 프로젝트로 분할 고려
* Chloros+ 병렬 처리로 결과 가속화
* CLI 또는 API로 배치 자동화 고려
* 최소 재보정 간격 조정으로 타겟 탐지 시간 단축
