...
* 다중 CPU 코어 활용
* NVIDIA 그래픽 카드 사용 시 선택적 GPU(CUDA) 가속 지원
...
* **마우스 오버** 시 상세 4단계 드롭다운 패널 표시
* **클릭** 시 드롭다운 패널 고정
* **다시 클릭**하면 패널이 해제되고 숨겨집니다
...
{% hint style=&quot;info&quot; %}
**Chloros+ 속도**: 대규모 데이터셋에서 병렬 처리는 순차 모드보다 5~10배 빠를 수 있습니다. 무료 모드에서 2시간 걸리는 500개 이미지 프로젝트가 Chloros+로 15~20분 내에 완료될 수 있습니다.
{% endhint %}
...
모니터링에 대한 자세한 내용은 [프로세싱 모니터링](monitoring-the-processing.md)을 참조하십시오.
...
1. **중지/취소 버튼** 찾기 (처리 중에는 시작 버튼으로 대체됨)
2. 중지 버튼 클릭
3. 처리가 즉시 중단됨
4. 부분적인 결과는 폐기됨
...
{% hint style=&quot;info&quot; %}
**첫 실행**: Chloros가 캐시와 프로필을 구축하는 초기 처리에는 더 오랜 시간이 소요될 수 있습니다. 이후 유사한 데이터셋의 처리는 더 빨라집니다.
{% endhint %}
...
1. 백엔드가 완전히 초기화될 때까지 기다림 (메인 메뉴 아이콘 확인)
2. 파일 브라우저에서 이미지가 가져온 상태인지 확인
3. 버튼이 계속 비활성화된 경우 Chloros 재시작
4. 디버그 로그에서 오류 메시지 확인
...
2. **사용 가능한 디스크 공간 확인** - 데이터셋 크기의 2~3배 여유 확보
...
3. **진행 상황 수시 확인** - 경고 또는 오류 발생 여부 점검
