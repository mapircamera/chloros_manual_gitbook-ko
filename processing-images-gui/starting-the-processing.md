...
**소요 시간:** 1~30초(표시된 타겟), 5~30분 이상(표시되지 않은 경우)
...
* 모든 밴드/채널에 걸쳐 보정 적용
* 타임스탬프 기반으로 각 이미지에 적합한 보정 대상 사용
...
* 잘못된 설정을 사용한 것을 인지한 경우
* 대상 이미지 표시를 잊은 경우
* 잘못된 이미지를 가져온 경우
* 시스템이 너무 느리게 실행되거나 응답하지 않는 경우
...
* 필요에 따라 설정을 조정하십시오
* 처리를 처음부터 다시 시작하십시오
* 최상의 경험을 위해 Chloros를 완전히 종료하고 재시작하십시오

{% hint style=&quot;warning&quot; %}
**부분 결과 없음**: 취소 시 모든 진행 상황이 삭제됩니다. Chloros는 부분적으로 처리된 이미지를 저장하지 않습니다.
{% endhint %}
...
{% hint style=&quot;info&quot; %}
**첫 실행**: Chloros가 캐시와 프로필을 구축하는 초기 처리에는 더 오랜 시간이 소요될 수 있습니다. 이후 유사한 데이터셋의 처리 속도는 빨라집니다.
{% endhint %}
